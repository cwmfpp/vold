...
#include <net/if.h> //cwm add
#include <linux/if.h> //cwm add
...
//using android::base::ParseInt; //cwm del
...
bool NetlinkEvent::parseNdUserOptMessage(const struct nlmsghdr *nh) {
    ...
    if (opthdr->nd_opt_type == ND_OPT_RDNSS) {
        ...
        free(buf);
    }  else if (opthdr->nd_opt_type == ND_OPT_DNSSL) {//cwm del
        // TODO: support DNSSL.
