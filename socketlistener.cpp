...
int SocketListener::startListener(int backlog) {
  ...
  callstack.log("callstack-tag-SocketListener");
        SLOGE("%d %s chenwenmin \n", __LINE__, __FUNCTION__);
#endif


    SLOGE("%d %s chenwenmin mSock=%d  mSocketName=%s\n", __LINE__, __FUNCTION__, mSock, mSocketName);
    SLOGE("%d %s chenwenmin mListen=%d  \n", __LINE__, __FUNCTION__, mListen);
    ...
    if (!mSocketName && mSock == -1) {
        ...
        if ((mSock = socket(AF_INET, SOCK_STREAM, PF_INET)/*android_get_control_socket(mSocketName)*/) < 0) {
    ...
    SLOGE("%d %s chenwenmin mUseCmdNum=%d  \n", __LINE__, __FUNCTION__, mUseCmdNum);
